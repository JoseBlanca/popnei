...
/// asked the machine for its memory with `try_reserve_exact` and gives its
/// values out as a slice, so there is no allocation to hand over to numpy as
/// the distances of `dists.rs` hand theirs. What the copy costs is the
/// matrix again while it runs, 200 MB at the 5000 variants of
/// `DEFAULT_MAX_NUM_VARS` and 8 bytes a pair at any other number; a core
/// that gave its `Vec` away instead would save it.
///
/// # Errors
///
...
fn the_square_of<'py>(
    ...
    let square =
        Array2::from_shape_vec((num_vars, num_vars), matrix.r2().to_vec()).map_err(|error| {
