"""It writes the VCF of 100000 variants x 1000 individuals that task 5.2 of
docs/plans/vcf-to-blocks.md times the reader on.

The genotypes are simulated by `simulate_genotypes` of
test/gwas_reference/make_reference.py of pyNei, copied here unchanged, with
NUM_SAMPLES 1000 and NUM_VARS 100000 in the place of the 200 and the 1200 of
that file, which is the panel section 6 of docs/rust_core.md names. The VCF
is written by `write_vcf` of the same file: the same header, the same nine
first columns with `.` in QUAL, FILTER and INFO, `A` and `T` as the alleles,
and `./.` where a genotype is missing. Here it writes the same bytes through
numpy instead of one f-string per genotype, because 100 million f-strings
take minutes.
...
drawn. Everything else, the seed included, is the script's.
    ...
    uv run --no-project --with numpy python make_big_vcf.py <out.vcf>
"""
...
def main():
    ...
    alleles, _pops = simulate_genotypes(rng)
