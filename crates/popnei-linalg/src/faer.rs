...
/// a time, from the largest eigenvalue, while the BLAS one has `dsyevd`
/// overwrite that buffer in place. What a caller sees is the same.
///
/// # Errors
///
...
pub(crate) fn eigh_lower(g: Vec<f64>, n: usize) -> Result<Eigen> {
    ...
    // faer gives the eigenvalues from the smallest, as LAPACK does, so
    // both the values and the vectors are read from the last.
    let eigenvalues = decomposition.S();
    let eigenvalues = eigenvalues.column_vector();
    let eigenvectors = decomposition.U();
    let values: Vec<f64> = (0..n).rev().map(|column| eigenvalues[column]).collect();
    let mut vectors = g;
    for (row, column) in vectors.chunks_exact_mut(n).zip((0..n).rev()) {
