...
`From<PyPopneiError> for PyErr`. Its six cases are the error of the core;
...
before the core sees it; a defect of this crate, a lock that a panic left
broken or a chromosome that is not in the table it came from; and an
exception the interpreter itself raised, the `KeyboardInterrupt` that
`py.check_signals` finds between two blocks, which travels back as it is.
